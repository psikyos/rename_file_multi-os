...
{
	printf("Hello, world\n");
	char *param_filename=(char *)"./sns.ini";
		...
		else	//否则是文件则都给改名
		{
			if(strcmp(dp->d_name,".")!=0 && strcmp(dp->d_name,"..")!=0)//如果不是.和..
			{
				if(strstr(dp->d_name,fileext)==NULL)//如果后缀名不是.jpg
				{
					sprintf(oldname,"%s/%s",basedir,dp->d_name);
					printf("%s\n",oldname);
					sprintf(newname,"%s/%s.jpg",basedir,dp->d_name);//追加后缀名
...
int slash_jpg(char *basedir,char *fileext)
{
	...
	for(dp = readdir(dfd); NULL!=dp; dp = readdir(dfd))
    {
		...
		stat(oldname, &s);
		//if(dp->d_type == DT_DIR&& ( strcmp(dp->d_name,".")!=0 && strcmp(dp->d_name,"..")!=0 ) )//如果是目录且不是.和..
		if( ((s.st_mode & S_IFMT) == S_IFDIR) && ( strcmp(dp->d_name,".")!=0 && strcmp(dp->d_name,"..")!=0 ) )
		{
...
FileParameter ini_file_read(char *para_file)
{
    ...
    { 
		...
		fparam.directory=config_profile_string(buffer,thefz,(char *)"directory=");
		fparam.extension=config_profile_string(buffer,thefz,(char *)"extension=");
		fclose(fp);
	}
	else
